...
#太阳能数据集
class Solar_Power_Data(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='Solar_Power_Plant_Data.csv',
                 target='SystemProduction', scale=True, timeenc=0, freq='h', cycle=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        df_raw.rename(columns={"Date-Hour(NMT)": "date"}, inplace=True)
        border1s = [0, 7 * 30 * 24 - self.seq_len, 9 * 30 * 24 - self.seq_len]
        border2s = [7 * 30 * 24, 9 * 30 * 24, 11 * 30 * 24]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
